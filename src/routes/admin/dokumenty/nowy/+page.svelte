<script lang="ts">
	// „Nowy dokument" (04.1-UI-SPEC Component Contract 5).
	//
	// FIXED DOM ORDER, and it is the contract's: back link, h1, save-result region, validation
	// summary region, required-fields note, field groups, save row. The two regions are one
	// surface here, because to an editor a refusal is one answer: „this did not happen, and
	// here is what to do about it".
	//
	// THE FORM SUPPRESSES THE BROWSER'S OWN VALIDATION BUBBLES, which are drawn in the
	// browser's locale and not in Polish. The `required` attributes stay for semantics and for
	// mobile keyboards; the server is the validation that matters and is the only thing that
	// can refuse a save. The attribute that does the suppressing appears exactly once in this
	// file, on the form element, and is deliberately not named here: the acceptance gate
	// counts its occurrences.
	//
	// WITHOUT JAVASCRIPT this screen is complete APART FROM ATTACHING THE FILE (P-22), which
	// the file group says out loud in its own no-script panel. Every other field posts,
	// validates and re-renders with the typed values intact, and a successful save is a
	// redirect the browser follows on its own. The saving state below is an ENHANCEMENT a
	// hydrated page switches on; it is never how the screen works.
	//
	...
	import { KOPIA_EKRAN_DOKUMENTU, KOPIA_LISTY, KOPIA_ZAPIS } from '$lib/content/panel';
	...
	// and a refused submission is the values it hands back; a plain `$state(...)` would capture
	// only the FIRST render and ignore every later server answer, which is exactly the defect
	// 04.1-05 hit on the nabór radios.
	let wartosci = $derived(form?.wartosci ?? data.wartosci);
	const pola = $derived(form?.pola ?? {});

	/** Saving state of Contract 9. Only ever true on a hydrated page; the server-rendered row
	 *  is a plain working button. */
	let zapisywanie = $state(false);

	/** In-page links from the summary to each offending control. The ids are the ones
	 *  PolaDokumentu derives, and the version group's link points at its first select, because
	 *  a fieldset is not focusable and „go to the date" means „go to the day". */
...
<!-- 2. The one h1. -->
<h1 class="naglowek">{KOPIA_EKRAN_DOKUMENTU.nowyNaglowek}</h1>
...
>
	...
	<!-- 6. Field groups. -->
	<PolaDokumentu {wartosci} {pola} lata={data.lata} kategorie={data.kategorie} />

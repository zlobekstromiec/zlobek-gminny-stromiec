...
- **Email:** free MailChannels is dead (2024) → Resend from a **verified sending domain**; parent's address goes in `reply-to`, never `from`; recipient hard-coded `publicznyzlobek@ugstromiec.pl`, with a hard-coded `cc` to the Urząd Gminy clerk.
...
- **Recipient mailbox:** submissions go to `publicznyzlobek@ugstromiec.pl`, the address the placówka gave in writing on 2026-08-27. It replaced an earlier address on the same domain that turned out never to have been a real mailbox, which is why the old "do not change the recipient" rule is gone: its premise expired. A `cc` copy goes to the Urząd Gminy clerk who runs recruitment casework; that recipient is a module constant in `mailer.ts` and is disclosed in the klauzula, and the two may never drift apart. The BCC backup `devzlobekstromiec@gmail.com` **stays** until delivery to the new mailbox is actually proven, and the proof is one form submission after deploy plus a look in the inbox, no deploy of its own.

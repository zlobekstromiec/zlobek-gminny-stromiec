...
| D-24 (FORM-02 parts B and C) and D-28 (UAT rows B4 **and B2**) | FORM-02, CMS-02 | Human sessions with no code component, zero deploys | Part B: one submission at or after the top of the next clock hour. Part C: one on a new UTC date. B4: a second editor in a second browser tab. **B2 (added 2026-08-18): upload a HEIC original from an iPhone through /admin and confirm `git show --stat` shows exactly two files sharing one basename.** B2 was in Phase 7 until the director's seven real photographs landed via quick task `260818-i5t`; its blocking input now exists. If those files turn out to be processed exports rather than camera originals, the HEIC assumption remains untested and the row must record that rather than pass |

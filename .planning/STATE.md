...
- [Phase 4 / 04-08] FORM-02 is UNMARKED again until the LIVE rate-limit ceilings are re-checked on the deployed site. The CR-01 bucketing fix (the window now lives in the KV key: hour of epoch per client, UTC calendar date site-wide) is unit proven with frozen-clock cases crossing both the hour and the UTC date boundary, but KV behaviour in the real edge environment is only observable in production. The re-check: submit /kontakt six times inside one clock hour and confirm the sixth returns 429, then confirm the same client is accepted again in the next clock hour without any period of site silence, and separately confirm a fresh `rl:doba:` key appears for the new UTC date. One human session, zero deploys.

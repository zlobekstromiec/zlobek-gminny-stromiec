...
**Success Criteria** (what must be TRUE):
  ...
  3. The five outstanding Deklaracja/RODO content classes are supplied by the Gmina and published: the koordynator dostępności contact, the IOD contact, the dostępność architektoniczna facts, the dostępność komunikacyjno-informacyjna facts (sign-language interpreter, hearing loop, assistance on request) and the pozostałe informacje section. The declaration then validates clean at `deklaracja-dostepnosci.info/walidator`, which is the independent oracle for A11Y-03. This closes A11Y-03 and LEGAL-02.

...
- [x] 05-06-PLAN.md — The gallery store, the galeria- naming and ownership rule, and /admin/galeria with the twelve cap
...
**Success Criteria** (what must be TRUE):
  ...
  2. Both mandatory legal pages are authored in full and published: a Deklaracja dostępności following the official government template (conformance status, procedura wnioskowo-skargowa, dostępność architektoniczna), and a Polityka prywatności / RODO page. The Deklaracja carries the full statutory structure required by Warunki techniczne v2.0 (binding 31 July 2024): all eight mandatory Polish headings and the fourteen mandatory `id="a11y-*"` identifiers the government monitoring crawler reads. Five content classes remain as marked placeholders, because no source for them exists in the project: the koordynator dostępności contact, the IOD's name and surname (NARROWED on 2026-08-27: the IOD's contact address arrived and is published, so only the name is still missing), the dostępność architektoniczna facts, the dostępność komunikacyjno-informacyjna facts, and the pozostałe informacje section (`dane-bip-zlobek-stromiec.md:196` scores this page ~10% and names the architectural facts as the gap). A11Y-03 and LEGAL-02 tick when Phase 7 supplies all five. NOTE: the Polityka prywatności was written and published early, on 2026-08-27, in quick task 260827-bfa, once the placówka supplied the administrator's klauzula. What this criterion still owes is therefore the Deklaracja dostępności alone.
...
**Success Criteria** (what must be TRUE):
  ...
  2. A live end-to-end test email confirms delivery to the recipient address `publicznyzlobek@ugstromiec.pl` (given 2026-08-27, sent from our domain `zlobekstromiec.pl`), which also closes FORM-01. The BCC backup mailbox and the klauzula paragraph that discloses it are removed in the same commit.
  3. The five outstanding Deklaracja/RODO content classes are supplied by the Gmina and published: the koordynator dostępności contact, the IOD's name and surname (the contact address itself arrived on 2026-08-27 and is already published; art. 11 of the ustawa of 10 May 2018 requires both), the dostępność architektoniczna facts, the dostępność komunikacyjno-informacyjna facts (sign-language interpreter, hearing loop, assistance on request) and the pozostałe informacje section. The declaration then validates clean at `deklaracja-dostepnosci.info/walidator`, which is the independent oracle for A11Y-03. This closes A11Y-03 and LEGAL-02.
...
| ~~Exact recipient email~~ — **RESOLVED 2026-08-27** | Phase 4, Phase 6 | **`publicznyzlobek@ugstromiec.pl`**, given in writing by the Urząd Gminy. It supersedes the previously "confirmed" `zlobek@` address on the same domain, which turned out never to have existed as a mailbox at all, so every earlier confirmation of it was wrong. A `cc` copy goes to the clerk running recruitment casework. Gmina/żłobek mailbox on `ugstromiec.pl`; Gmina/żłobek = data controller. |
| Gmina IT: allowlist our sending domain (`send.zlobekstromiec.pl` / From address) + confirm `publicznyzlobek@ugstromiec.pl` receives **external** mail | Phase 4 (test), Phase 6 (launch) | **Soft, non-blocking.** Government mail gateways filter aggressively; run an early real end-to-end test and monitor Resend delivery/bounce webhooks (or BCC a backup) so a filtered enrollment isn't silently lost. |
...
- **Security:** sanitize the submitted email before `Reply-To` (header-injection); Turnstile verified server-side; recipients hard-coded (`publicznyzlobek@ugstromiec.pl` plus a `cc` to the Urząd Gminy clerk, neither ever taken from the request body); rate-limit the endpoint.
...
| Koordynator dostępności / IOD contact details | Phase 4 (klauzula informacyjna), Phase 6 (Deklaracja dostępności) | Client must name a person. The IOD's contact ADDRESS was delivered on 2026-08-27 and is published; the inspector's name and surname are still owed, and art. 11 of the ustawa of 10 May 2018 requires both. The koordynator dostępności is unchanged: still nobody. |

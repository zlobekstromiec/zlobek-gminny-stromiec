/**
 * VENDORED, PINNED copy of the sveltia-cms-auth OAuth proxy Worker.
 *
 * Upstream: https://github.com/sveltia/sveltia-cms-auth
 * Pinned commit: cc7530f1787c851832ee2229be5c0e1e4ab7a154 (2026-08-05)
 * Fetched: 2026-08-13 (Plan 02-05)
 *
 * Why vendored: Cloudflare has no Netlify Git Gateway, so the GitHub OAuth
 * code -> token exchange must run server-side on a Worker that holds the
 * client secret. We self-host + pin this one-file Worker instead of hot-linking
 * upstream, so an upstream change can never silently alter our auth path
 * (supply-chain threat T-0205-SC). Refresh deliberately by re-fetching the file
 * at a new pinned commit.
 *
 * Configuration (see wrangler.toml + README.md):
 *   - GITHUB_CLIENT_ID     Worker secret (wrangler secret put) -- NEVER committed
 *   - GITHUB_CLIENT_SECRET Worker secret (wrangler secret put) -- NEVER committed
 *   - ALLOWED_DOMAINS      Worker var (wrangler.toml [vars]) = live Pages hostname
 *
 * This file is intentionally excluded from the SvelteKit project's
 * prettier/eslint gate (.prettierignore + eslint.config.js): it is vendored
 * third-party source with its own upstream formatting, not project code.
 */
